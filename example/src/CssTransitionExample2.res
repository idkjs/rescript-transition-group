...
module Styles = {
  open Css
  let alertEnter = style(list{opacity(0.), transform(#scale(0.9, 0.9)), label("alert-enter")})
      ...
      {_state =>
        <div className=Styles.modal>
          <div className=Styles.modalHead>
            <span className=Styles.modalTitle> {"Animated alert message" |> React.string} </span>
            <div onClick={_ => setShowMessage(_ => false)}> {"x" |> React.string} </div>
          </div>
          <span>
